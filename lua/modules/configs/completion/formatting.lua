...
vim.api.nvim_create_user_command("FormatToggle",
                                 function() M.toggle_format_on_save() end, {})
...
vim.api.nvim_create_user_command("FormatterToggle", function(opts)
    if block_list[opts.args] == nil then
        vim.notify(string.format(
                       ...
                       opts.args), vim.log.levels.WARN,
                   {title = "LSP Formatter Warning!"})
        block_list[opts.args] = true
    else
        block_list[opts.args] = not block_list[opts.args]
        vim.notify(string.format("[LSP]Formatter for [%s] has been %s.",
                                 opts.args, not block_list[opts.args] and
                                     "enabled" or "disabled"),
                   not block_list[opts.args] and vim.log.levels.INFO or
                       vim.log.levels.WARN, {
            title = string.format("LSP Formatter %s",
                                  not block_list[opts.args] and "Info" or
                                      "Warning")
        })
    end
end, {
    ...
    complete = function()
        return {
            "markdown", "vim", "lua", "c", "cpp", "python", "vue", "typescript",
            "javascript", "yaml", "html", "css", "scss", "sh", "rust"
        }
    end
})
...
function M.enable_format_on_save(is_configured)
    ...
    if not is_configured then
        vim.notify("Successfully enabled format-on-save", vim.log.levels.INFO,
...
function M.toggle_format_on_save()
    local status = pcall(vim.api.nvim_get_autocmds,
                         {group = "format_on_save", event = "BufWritePre"})
    if not status then
...
function M.format_filter(clients)
    return vim.tbl_filter(function(client)
        ...
        if status_ok and formatting_supported and client.name == "null-ls" then
            return "null-ls"
        elseif client.name ~= "lua_ls" and client.name ~= "tsserver" and
            client.name ~= "clangd" then
...
function M.format(opts)
    ...
    if #clients == 0 then
        vim.notify("[LSP]Format request failed, no matching language servers.",
                   vim.log.levels.WARN, {title = "Formatting Failed!"})
    end
    ...
    for _, client in pairs(clients) do
        if block_list[vim.bo.filetype] == true then
            vim.notify(string.format(
                           ...
                           client.name, vim.bo.filetype), vim.log.levels.WARN,
                       {title = "LSP Formatter Warning!"})
            return
        end
        local params = vim.lsp.util.make_formatting_params(
                           opts.formatting_options)
        local result, err = client.request_sync("textDocument/formatting",
                                                params, timeout_ms, bufnr)
        if result and result.result then
            vim.lsp.util.apply_text_edits(result.result, bufnr,
                                          client.offset_encoding)
            vim.notify(string.format("[LSP]Format successfully with [%s]!",
                                     client.name), vim.log.levels.INFO,
                       {title = "LSP Format Success!"})
        elseif err then
            vim.notify(string.format("[LSP][%s] %s", client.name, err),
                       vim.log.levels.ERROR, {title = "LSP Format Error!"})
        end
    end
end
